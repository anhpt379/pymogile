...
class Admin(object):
    ...
    def get_devices(self, devid=None):
        ...
        for x in xrange(1, int(res['devices']) + 1):
            device = {}
            for k in (
                'devid', 'hostid', 'status', 'observed_state', 'utilization'
            ):
    ...
    def _set_slave_keys(self, keys):
        ...
        self.backend.do_request(
            "set_server_setting", {'key': 'slave_keys',
                                   'value': ''.join(buf)}
        )
    ...
    def slave_list(self):
        ...
        for key in keys:
            res = self.backend.do_request(
                'server_setting', {'key': 'slave_%s' % key}
            )
            if not res:
                continue
    ...
    def slave_modify(self, key, **opts):
        ...
        value = '|'.join([dsn, username, password])
        res = self.backend.do_request(
            'set_server_setting', {'key': 'slave_%s' % key,
                                   'value': value}
        )
    ...
    def fsck_reset(self, policy_only, startpos):
        return self.backend.do_request(
            "fsck_reset", {'policy_only': policy_only,
                           'startpos': startpos}
        )
    ...
    def get_stats(self):
        ...
        if 'replicationcount' in res:
            ...
            for x in xrange(1, int(res['replicationcount']) + 1):
                ...
                (replication.setdefault(domain, {}).setdefault(cls, {})
                 )[devcount] = fields
